...
import forgoutEmail from "../api/forgoutEmail/forgoutEmail";
...
export default function Forgout() {
  ...
  const handlerForgout = async () => {
    try {
      await forgoutEmail({ email }).then(() => rota.push("/"));
    } catch (error) {
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        ShowToolBarMessage(error.response.data.message, ToolbarEnum.error);
      }
  ...
  return (
    <main className="flex flex-col justify-center items-center w-full h-full bg-background">
      <section className="flex flex-col justify-center items-center gap-8 w-[90%] backdrop-blur-sm px-4 py-10 border-orange-400 border-[0.1rem] rounded-lg">
